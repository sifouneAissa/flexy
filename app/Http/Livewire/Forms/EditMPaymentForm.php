<?php

namespace App\Http\Livewire\Forms;

use App\Models\PaymentMethod;
use App\Models\Provider;
use Livewire\Component;
use Livewire\WithFileUploads;

class EditMPaymentForm extends Component
{
    use WithFileUploads;

    public $item;
    public $m_h = 132;
    public $m_w = 178;
    public $photo;
    public $d_img;
    public $name;
    public $provider;
    public $description;
    public $himage = false;

    protected function rules(){
        $rules = [
            'name' => 'required|min:1|unique:providers',
            'provider' => 'required|min:1',
            'description' => 'required|min:5|max:255',
        ];

        $rules['photo'] = ($this->himage ? 'nullable|' : 'required|').'image|max:1024|dimensions:min_width='.$this->m_w.',min_height='.$this->m_h.',max_width='.$this->m_w.',max_height='.$this->m_h;


        return $rules;
    }



    public function mount(){
        $this->d_img = 'https://placehold.it/'.$this->m_w.'x'.$this->m_h;
        $this->name = $this->item->name;
        $this->provider = $this->item->provider;
        $this->description = $this->item->description;


        if($url = $this->item->getWebP()) {
            $this->d_img = $url;
            $this->himage = true;
        }
    }


    public function updated($propertyName)
    {
        $this->validateOnly($propertyName);
    }



    public function removeImage(){
        $this->photo = null;
        $this->resetValidation(['photo']);
    }


    public function save(){
        $state = $this->validate();

        startTransaction(function (){

            $this->item->update(filterRequest($this->all(),PaymentMethod::class));
            if($this->photo){
                // delete the old photo
                if($f = $this->item->fimage()) $f?->delete();
                $this->item->addMedia($this->photo)->toMediaCollection();
            }

            return redirect()->route('mpayment.index');
        });

    }

    public function render()
    {
        return view('livewire.forms.edit-m-payment-form');
    }
}
